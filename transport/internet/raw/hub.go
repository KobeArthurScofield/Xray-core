...
// ListenRAW creates a new Listener based on configurations.
func ListenRAW(ctx context.Context, address net.Address, port net.Port, streamSettings *internet.MemoryStreamConfig, handler internet.ConnHandler) (internet.Listener, error) {
	l := &Listener{
		addConn: handler,
	}
	rawSettings := streamSettings.ProtocolSettings.(*Config)
	l.config = rawSettings
	if l.config != nil {
	...
	if rawSettings.HeaderSettings != nil {
		headerConfig, err := rawSettings.HeaderSettings.GetInstance()
		if err != nil {
...
func init() {
	common.Must(internet.RegisterTransportListener(protocolName, ListenRAW))
}
